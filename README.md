<div align="center">

# 🪞 Echo Wall Breaker

### 🫧 回音壁解除 / 诚实对话校准

_支持你，但不盲目附和你。_

<br/>

[![中文](https://img.shields.io/badge/中文-当前语言-ffb4c8?style=for-the-badge)](./README.md)
[![English](https://img.shields.io/badge/English-Switch-94a3b8?style=for-the-badge)](./README.en.md)

<br/>

![type](https://img.shields.io/badge/type-AI%20Skill-8b5cf6?style=for-the-badge)
![focus](https://img.shields.io/badge/focus-dialogue%20calibration-f97316?style=for-the-badge)
![tone](https://img.shields.io/badge/tone-empathic%20%2B%20honest-22c55e?style=for-the-badge)
![status](https://img.shields.io/badge/status-ready-0ea5e9?style=for-the-badge)

</div>

---

## 🌸 这个 Skill 是什么？

**Echo Wall Breaker** 是一个用于减少 AI 对话中“回音壁效应”的轻量 Skill。  
它让 AI 不再只是顺着用户说“你完全对”“这个想法很棒”，而是更诚实、更温和、更可靠地参与讨论。

它的核心很简单：

> 🫶 支持用户这个人；  
> 🧭 校准用户的判断；  
> 🪞 不让 AI 变成一面只会附和的镜子。

---

## 🧸 它解决什么？

| 场景 | 常见问题 | 它会帮助你 |
|---|---|---|
| 💬 讨论 idea | AI 只会夸“很有潜力” | 提醒证据、风险、反例和边界 |
| 😣 情绪倾诉 | AI 无原则站队 | 承认感受，但不自动判定谁完全正确 |
| ⚖️ 人际判断 | 用户希望 AI 帮忙“定罪” | 区分事实、感受、解释和行动 |
| 🚀 过度乐观 | 只看成功路径 | 检查失败条件和潜在盲点 |
| 🌧️ 过度悲观 | 把一次挫折放大成整体否定 | 帮助拆出真实问题与情绪放大部分 |

---

## 🧩 核心能力

| 模块 | 作用 |
|---|---|
| 🪞 反回音壁 | 避免 AI 只重复和放大用户已有判断 |
| 🍬 反奉承 | 不用空泛赞美替代真实分析 |
| 🧠 层次拆分 | 区分事实、感受、解释、判断和行动 |
| 🔍 盲点提示 | 主动寻找反例、风险和替代解释 |
| 🫶 温和校准 | 先共情，再诚实指出不确定或不成立的部分 |

---

## 🍃 使用前后对比

| Before | After |
|---|---|
| “你完全没错。” | “你的感受可以理解，但事情判断还需要更多证据。” |
| “这个 idea 很强。” | “它有一个切入点，但贡献、baseline 和验证方式还要压实。” |
| “都是对方的问题。” | “我们先区分具体行为、你的感受和可能解释。” |
| “你肯定能成功。” | “有机会，但这里有三个现实风险需要先看清。” |

---

## 📦 安装方式

本仓库计划提供两种安装形式：

```text
echo-wall-breaker.skill
echo-wall-breaker/SKILL.md
```

根据你的平台选择单文件版或 `SKILL.md` 文件夹版。

---

## 🏷️ 推荐仓库简介

```text
A cute but honest dialogue-calibration Skill that reduces AI sycophancy, echo chambers, and blind validation.
```

## 🧷 推荐 topics

```text
ai-skill, dialogue-calibration, anti-sycophancy, echo-chamber, prompt-engineering, chatgpt-skill
```

---

<div align="center">

🫧 _Kindness is not the same as blind agreement._

</div>
